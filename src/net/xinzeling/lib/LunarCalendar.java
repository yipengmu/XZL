...
public class LunarCalendar {
	...
	public LunarCalendar caculate(){
		int leapMonth = 0;
		// 求出和1900年1月31日相差的天数
		int offset = (int) ((calendar.getTime().getTime() - baseTime) / 86400000L);
		...
		for (iMonth = 1; iMonth < 13 && offset > 0; iMonth++) {
			...
			offset -= daysOfMonth;
			if (leap && iMonth == (leapMonth + 1))// 解除闰月
				leap = false;
		}
	...
	public LunarCalendar(Calendar cal) {
		calendar = cal;
		try {
			baseTime = chineseDateFormat.parse("1900年1月31日").getTime();
	...
	public String getDay() {
		return cyclicalm(year) + "年" + (leap ? "闰" : "")+ chineseNumber[month - 1] + "月" + getChinaDay();
	}
}